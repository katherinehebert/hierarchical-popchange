...
saveRDS(mod1, paste0("outputs/testing/gam_01_nolatents.rds")) 

# Plot the random effect on species --------------------------------------------

png("outputs/testing/fig_gam_01_nolatents_randomeffect.png", height=1000, width=1800, type = "cairo",pointsize = 20)
mvgam::plot_mvgam_randomeffects(mod1)
dev.off()

# Plot the time smooth ---------------------------------------------------------

png("outputs/testing/fig_gam_01_nolatents_smooth.png", height=1000, width=1800, type = "cairo",pointsize = 20)
mvgam::plot_mvgam_smooth(mod1)
dev.off()


preds = mod1$mgcv_model |> predict(type = "response", se.fit = TRUE)
newDF <- with(mod1$mgcv_model$model, data.frame(time = unique(time)))
eps <- 1e-7
X0 <- predict(mod1$mgcv_model, type = 'lpmatrix')
newDFeps_p <- newDF + eps
X1 <- predict(mod1$mgcv_model, newDFeps_p, type = 'lpmatrix')
...
# Get derivative of predicted trends -------------------------------------------

source("~/Documents/GitHub/hierarchical-lpi/plot_mvgam_smooth_custom.R")

derivs_ls = list()
for(i in 1:npops){
  derivs_ls[[i]] = plot_mvgam_trend_custom(mod1, smooth = "time", 
                                            derivatives = TRUE, series = i)
}
derivs = do.call(rbind, derivs_ls)
saveRDS(derivs, "outputs/testing/gam_01_nolatents_derivatives.rds")
...
saveRDS(derivs_pops_df, "outputs/testing/gam_01_nolatents_derivs_pops_df.rds")
...
saveRDS(avg_deriv_trend, "outputs/testing/gam_01_nolatents_df_overall_deriv.rds")
...
saveRDS(temporal_trend_yearly, "outputs/testing/gam_01_nolatents_temporal_trend_yearly.rds")
...
(plot_coherence = ggplot(data = temporal_trend_yearly) +
   ...
   labs(y = "Community variability (σ)", 
        x = "",
        col = "σ") +
   scale_color_distiller(palette = "YlGnBu", direction = 1, limits = c(0, .2)) +
...
ggsave("outputs/testing/fig_02_onlylatents_assemblagevariability.png", width = 7.5, height = 7)
