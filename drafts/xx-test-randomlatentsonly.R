...
mod1 <- mvgam(data = data_train,
              ...
              use_lv = TRUE,
              family = "gaussian",
              trend_model = "RW", # RW(cor = TRUE), # multivariate random walk
              use_stan = TRUE,
              chains = 3, 
              burnin = 500,
              samples = 5000
)
saveRDS(mod1, paste0("outputs/testing/gam_03_onlyrandomlatents.rds")) 

# Plot latent factors ----------------------------------------------------------

png("outputs/testing/fig_gam_03_onlyrandomlatents_latentfactors.png", height=1000, width=1800, type = "cairo",pointsize = 20)
mvgam::plot_mvgam_factors(mod1)
dev.off()

# Plot the random effect on species --------------------------------------------

png("outputs/testing/fig_gam_03_onlyrandomlatents_randomeffect.png", height=1000, width=1800, type = "cairo",pointsize = 20)
...
saveRDS(preds, "outputs/testing/gam_03_onlyrandomlatents_predictions.rds")
saveRDS(derivs, "outputs/testing/gam_03_onlyrandomlatents_derivatives.rds")
...
saveRDS(derivs_pops_df, "outputs/testing/gam_03_onlyrandomlatents_derivs_pops_df.rds")
...
saveRDS(avg_deriv_trend, "outputs/testing/gam_03_onlyrandomlatents_df_overall_deriv.rds")
...
saveRDS(temporal_trend_yearly, "outputs/testing/gam_03_onlyrandomlatents_temporal_trend_yearly.rds")
...
ggsave("outputs/testing/fig_03_onlyrandomlatents_assemblagevariability.png", width = 7, height = 3)
...
(plot_coherence = ggplot(data = temporal_trend_yearly) +
   ...
   labs(y = "Community variability (σ)", 
        x = "",
        col = "σ") +
   scale_color_distiller(palette = "YlGnBu", direction = 1, limits = c(0, .37)) +
