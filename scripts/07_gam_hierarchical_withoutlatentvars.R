...
saveRDS(mod1, paste0("outputs/gam_hierarchical_rwlv.rds")) # 4% ended in divergence
...
saveRDS(posterior_df, "outputs/gam_hierarchical_posterior_rwlv.rds")

coefs_summ = coef(m, summarise = TRUE) |> as.data.frame()
saveRDS(coefs_summ, "outputs/gam_hierarchical_coefs_summary_rwlv.rds")
...
saveRDS(coef_df, "outputs/gam_hierarchical_coefs_rwlv.rds")


## gotta run from here down ----
...
saveRDS(predictions, "outputs/gam_hierarchical_pred_l_rwlv.rds")
...
ggsave("figures/mvgam_predictions_perspecies_rwlv.png", width = 12.8, height = 7.63)
...
ggsave("figures/distribution_poptrends_rwlv.png", width = 8.23, height = 9)
...
ggsave("figures/mvgam_rateofchange_rwlv.png", width = 11, height = 4.06)
...
saveRDS(avg_trend, "outputs/gam_hierarchical_df_overall_rwlv.rds")
...
# )
saveRDS(avg_deriv_trend, "outputs/gam_hierarchical_df_overall_deriv_rwlv.rds")
...
ggsave("figures/mvgam_prediction_rwlv.png", width = 8.46, height = 4.06)


# get species correlations
sp_correlations = lv_correlations(mod1)
saveRDS(sp_correlations, "outputs/gam_hierarchical_species_correlations_rwlv.rds")

# sp_correlations = readRDS("outputs/gam_hierarchical_species_correlations.rds")

# Plot as heatmap
png(height=1800, width=1800, file="figures/species_associations_rwlv.png", type = "cairo")
