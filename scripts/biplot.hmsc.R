...
#' @param randomeff_names Names of the random effect levels for plotting text - added by KH.
#' @param species_names Names of the species for plotting text - added by KH.
...
#' @export
biplot.hmsc <-
function(x, Random, choice = c(1, 2), display = c("sites", "species"), type, xlim, ylim, xlab, ylab, col = c(1, 2) , randomeff_names, species_names, ...){
...
##########################################################################################
	...
	if(length(choice)!=2){
		stop("'choice' should have a length of 2")
	}
  ...
  abline(v = 0, lty = 3)
    ...
    if(type[1] == "text"){
			text(latentMean, randomeff_names, col = col[1], cex = 0.7)
		}
  }
    ...
    if(type[2] == "text"){
			text(paramLatentMean, species_names,col = col[2], cex = 0.7)
		}
  }
