...
#' @export
predictQ.hmsc<-function(object, newdata, type = c("response","link"), conditional = NULL, nsample, ...){
  ...
  if(is.null(conditional)){
    ...
    if(type=="response"){
      if(any(class(object)=="probit")){
        ...
        overall_mu <- apply(res, 1, mean)
        overall_q025 <- apply(res, 1, function(x) {
          pnorm(quantile(x, .025))
        })
        overall_q975 <- apply(res, 1, function(x) {
          pnorm(quantile(x, .975))
        })
        result <- list(mu = mu, q025 = q025, q05=q05, q10 = q10, q90 = q90, q95 = q95, q975 = q975,
                       overall_mu = overall_mu, overall_q025 = overall_q025, overall_q975 = overall_q975)
      }
    ...
    if(type=="link"){
      ...
      ## this is the part I added myself to get mean at each time step with CI
      overall_mu <- apply(res, 1, mean)
      overall_q025 <- apply(res, 1, function(x) {
        quantile(x, .025)
      })
